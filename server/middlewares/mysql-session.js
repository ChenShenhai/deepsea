'use strict' ;
const co = require('co');
const mysql = require('co-mysql');
const FORTY_FIVE_MINUTES = 45 * 60 * 1000 ;

let tableName = 'session_store';
let CREATE_STATEMENT = 'CREATE  TABLE IF NOT EXISTS `' + tableName + '` (`id` VARCHAR(255) NOT NULL, `expires` BIGINT NULL, `data` TEXT NULL, PRIMARY KEY (`id`));';
let GET_STATEMENT = 'SELECT * FROM `' + tableName + '` WHERE id  = ?';
let SET_STATEMENT ='INSERT INTO ' + tableName + '(id, expires, data) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE expires=?, data =?';
let DELETE_STATEMENT = 'DELETE FROM `' + tableName + '` WHERE id  = ?';
let CLEANUP_STATEMENT = 'DELETE FROM `' + tableName + '` WHERE expires  < ?' ;
...
const MysqlStore = function (options) {
